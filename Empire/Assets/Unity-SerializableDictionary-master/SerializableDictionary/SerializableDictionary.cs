...
public abstract class SerializableDictionaryBase<TKey, TValue, TValueStorage> : SerializableDictionaryBase, IDictionary<TKey, TValue>, IDictionary, ISerializationCallbackReceiver, IDeserializationCallback, ISerializable
{
    ...
    public SerializableDictionaryBase() => m_dict = new Dictionary<TKey, TValue>();

    public SerializableDictionaryBase(IDictionary<TKey, TValue> dict) => m_dict = new Dictionary<TKey, TValue>(dict);
    ...
    public void Add(TKey key, TValue value) => ((IDictionary<TKey, TValue>)m_dict).Add(key, value);
    ...
    public bool TryGetValue(TKey key, out TValue value) => ((IDictionary<TKey, TValue>)m_dict).TryGetValue(key, out value);

    public void Add(KeyValuePair<TKey, TValue> item) => ((IDictionary<TKey, TValue>)m_dict).Add(item);

    public void Clear() => ((IDictionary<TKey, TValue>)m_dict).Clear();
    ...
    public void Add(object key, object value) => ((IDictionary)m_dict).Add(key, value);
    ...
    public void Remove(object key) => ((IDictionary)m_dict).Remove(key);

    public void CopyTo(Array array, int index) => ((IDictionary)m_dict).CopyTo(array, index);

    #endregion

    #region IDeserializationCallback

    public void OnDeserialization(object sender) => ((IDeserializationCallback)m_dict).OnDeserialization(sender);

    #endregion

    #region ISerializable

    protected SerializableDictionaryBase(SerializationInfo info, StreamingContext context) => m_dict = new Dictionary<TKey, TValue>(info, context);

    public void GetObjectData(SerializationInfo info, StreamingContext context) => ((ISerializable)m_dict).GetObjectData(info, context);
