...
public class ConvolutionalNeuralNetwork : MonoBehaviour
{
    ...
    [SerializeField]
    private ConvolutionalLayer convolutionalLayerPrefab;
    [SerializeField]
    private PoolingLayer poolingLayerPrefab;
    public List<ConvolutionalNetworkLayer> layers;
    private void Awake() => neuralNetwork = GetComponent<NeuralNetwork>();
    public void CreateNeuralNetwork(int widthInput, int heightInput, int[] countHiddenNeurons, int countOutputNeurons)
    {
        ...
        for (int id = 0; id < 2; id++)
        {
            ...
            layers.Last().Create(id == 0 ? widthInput : layers[^2].outWidth, id == 0 ? heightInput : layers[^2].outHeight, 3, 3);

            layers.Add(Instantiate(poolingLayerPrefab, transform));
            layers.Last().Create(layers[^2].outWidth, layers[^2].outHeight, 2, 2);
        }

        neuralNetwork.CreateNeuralNetwork(layers.Last().outWidth * layers.Last().outHeight, countHiddenNeurons, countOutputNeurons);
    }
